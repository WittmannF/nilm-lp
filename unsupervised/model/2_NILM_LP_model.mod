...
param standby_state;            # Estado que estara sempre ligado. As restricoes de tempo nao serao consideradas neste caso
...
subject to set_zero {t in TS2, e in ESTADO : Ptotal[t] < TH and e!=standby_state}:
	X[e,t] = 0;

# Definir um estado sempre ligado (standby)
subject to standby_power {t in TS2}:
        X[standby_state,t] = 1;
...
subject to numero_minimo_amostras_1 {e in ESTADO, t in G[e] + disc_i .. (Tf - mindisc[e] + 1) : t>disc_i and e!=standby_state}:
	sum{n in t..(t + mindisc[e] - 1)} (X[e,n]) >= mindisc[e]*(X[e,t] - X[e,t-1]);

subject to numero_minimo_amostras_2 {e in ESTADO : e!=standby_state}:
	sum{n in disc_i .. G[e]} (1 - X[e,n]) = 0;

subject to numero_minimo_amostras_3 {e in ESTADO, t in (Tf - mindisc[e] + 2) .. Tf : e!=standby_state}:
